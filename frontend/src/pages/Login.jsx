...
const Login = ({ onLogin }) => {
    ...
    const loginWithGoogle = useGoogleLogin({
        ...
        onError: (errorResponse) => {
            ...
            setError('Google Login Failed: ' + (errorResponse?.error_description || errorResponse?.error || 'Popup closed or access denied.'));
            setLoading(false);
        }
    });
                ...
                <div className="relative z-10">
                    ...
                    >
                        <h2 className="text-6xl font-black text-white leading-none mb-8">
                            Writing that <br />
                            <span className="text-rose-200 italic underline decoration-rose-300/50 underline-offset-8">feels</span> human.
                        </h2>
                        <p className="text-indigo-50/90 text-xl max-w-md leading-relaxed font-medium">
                            The space where your best ideas come to life. Real-time, intelligent, and beautifully intuitive.
                        </p>
                    </motion.div>
                </div>

                <div className="relative z-10">
                    <div className="flex gap-6 items-center bg-white/10 backdrop-blur-md p-4 rounded-3xl border border-white/20 w-fit">
                        <div className="flex -space-x-3">
                            {[1, 2, 3, 4].map(i => (
                                <div key={i} className={`w-12 h-12 rounded-full border-4 border-indigo-500 bg-gradient-to-br ${i % 2 === 0 ? 'from-rose-400 to-rose-500' : 'from-indigo-400 to-indigo-500'} flex items-center justify-center text-sm font-bold text-white shadow-xl`}>
                                    {String.fromCharCode(64 + i)}
                                </div>
                            ))}
                        </div>
                        <div>
                            <p className="text-white text-base font-bold">15k+ Authors</p>
                            <p className="text-indigo-100/70 text-xs font-medium uppercase tracking-widest">Collaborating now</p>
                        </div>
                    </div>
                </div>

                {/* Decorative Elements */}
                <div className="absolute top-[-10%] right-[-10%] w-[500px] h-[500px] bg-rose-400/20 rounded-full blur-[120px]" />
                <div className="absolute bottom-[-20%] left-[-10%] w-[600px] h-[600px] bg-indigo-400/20 rounded-full blur-[150px]" />
            </div>
            ...
            <div className="w-full lg:w-1/2 flex items-center justify-center p-8 lg:p-16">
                ...
                >
                    ...
                    <div className="mb-12 text-center lg:text-left">
                        <h1 className="text-5xl font-black text-brand-text mb-4 tracking-tight">Welcome Back</h1>
                        <p className="text-gray-500 text-lg font-medium">Continue your writing journey today.</p>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-8">
                            ...
                            <div className="relative group">
                                <input 
                                    ...
                                    className={`w-full px-6 py-5 rounded-3xl bg-white border-2 outline-none transition-all font-bold text-gray-700 shadow-sm placeholder:text-gray-300 ${
                                        ...
                                        : 'border-gray-100 focus:border-brand-primary focus:ring-8 focus:ring-brand-primary/5'
                                    }`}
                        ...
                        <div className="space-y-3">
                            <div className="flex justify-between items-center px-1">
                                ...
                                <Link to="/forgot-password" global="true" className="text-sm font-black text-brand-primary hover:text-indigo-700 transition-colors">Forgot?</Link>
                            </div>
                            <div className="relative group">
                                <input 
                        ...
                        <div className="flex items-center gap-3 px-1">
                            <input type="checkbox" id="remember" className="w-5 h-5 rounded-lg border-2 border-gray-200 text-brand-primary focus:ring-brand-primary transition-all cursor-pointer" />
                            <label htmlFor="remember" className="text-base font-bold text-gray-500 cursor-pointer">Stay signed in</label>
                        </div>
                    ...
                    <p className="mt-12 text-center text-lg font-bold text-gray-400">
                        New here? <Link to="/signup" className="text-brand-primary font-black hover:text-indigo-700 underline underline-offset-8 decoration-4 decoration-brand-primary/30 hover:decoration-brand-primary transition-all">Create an account</Link>
                    </p>
                </motion.div>
            </div>
        </div>
